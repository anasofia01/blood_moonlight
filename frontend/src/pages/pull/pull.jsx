...
const PullPage = () => {
	...
	useEffect(() => {
		...
		socket.emit('getRoomState', { roomId }, (room) => {
			if (!room) return;
			const alivePlayers = room.players.filter((p) => p.alive && p.id !== playerId);
			setPlayers(alivePlayers);
			setRound(room.round ?? round);
		});

		const handleRoomUpdate = (updatedPlayers) => {
			const alivePlayers = updatedPlayers.filter((p) => p.alive && p.id !== playerId);
			setPlayers(alivePlayers);
		};
		...
		const handleVotingEnded = ({ eliminated, wasInfected, updatedPlayers = [], nextRound }) => {
			const alivePlayers = updatedPlayers.filter((p) => p.alive && p.id !== playerId);
			setPlayers(alivePlayers);
		...
		const handleGameEnded = ({ winnerId, winnerName }) => {
			setGameEnded(true);
			localStorage.setItem('gameWinner', JSON.stringify({ winnerId, winnerName }));
			navigate('/tablero', { replace: true, state: { continueGame: false } });
		};
		...
		<div className='page'>
			{gameEnded ? null : !isVotingRound ? (
				<div className='waiting-container'>
					...
					<img src='/images/murcielago.png' alt='Vampiro' className='waiting-image' />
				</div>
			) : (
				<>
